...
public class CalculationService : ICalculationService
{
    ...
    private readonly IAnomalyPriceRepository _anomalyPriceRepository;

    public CalculationService(
        ICalculationRepository calculationRepository,
        IGoodsRepository goodsRepository,
        IAnomalyPriceRepository anomalyPriceRepository)
    {
        _calculationRepository = calculationRepository;
        _goodsRepository = goodsRepository;
        _anomalyPriceRepository = anomalyPriceRepository;
    }
    ...
    public async Task SaveAnomalyPrice(
        SaveAnomalyPriceModel model,
        CancellationToken token)
    {
        using (TransactionScope transactionScope = _anomalyPriceRepository.CreateTransactionScope())
        {
           await _anomalyPriceRepository.Save( 
                new SaveAnomalyPriceEntityV1(
                    model.GoodId,
                    model.Price),
                token);
